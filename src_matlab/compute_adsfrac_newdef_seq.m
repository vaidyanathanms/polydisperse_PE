...
for pdi_cntr = 1:length(pdi_freearr) % begin pdi free loop
    ...
    fout_avg = fopen(sprintf('./../../seq_analysis/overall/adsorbedmon_wrtch_ave_allcases_rcut_%s_pdifree_%g.dat',...
        cutoff,pdifree),'w');
    ...
    fout_sidata = fopen(sprintf('./../../seq_analysis/overall/sidata_adsorbedmon_wrtch_rcut_%s_pdifree_%g.dat',...
        cutoff,pdifree),'w');
    ...
    for ncnt = 1:length(nfreearr) % begin nfree loop
        ...
        for arch_cnt = 1:length(arch_arr)  % begin arch loop
            ...
            fout_case = fopen(sprintf('./../../seq_analysis/n_%d/adsorbedmon_wrtch_rcut_%s_pdifree_%g_%s.dat',...
