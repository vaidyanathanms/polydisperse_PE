%% To compute and plot number averaged MW
% Run generate_numavgMW.m before running this
clc;
close all;
clear;
format long
...
for pdi_cntr = 1:length(pdi_freearr) % begin pdi free loop
    ...
    fout_avg = fopen(sprintf('./../../seq_analysis/overall/numavgMW_ave_allcases_rcut_%s_pdifree_%g.dat',...
        cutoff,pdifree),'w');
    fprintf(fout_avg,'%s\t%s\t%s\t%s\t%s\t%s\n','N_f','Arch','ncases','numsample_pts',...
        'avg_fraction_numavgMW','StdErrMean');
    
    % Create case-based avg seq_analysis for SI data
    fout_sidata = fopen(sprintf('./../../seq_analysis/overall/sidata_numavgMW_rcut_%s_pdifree_%g.dat',...
        cutoff,pdifree),'w');
    fprintf(fout_sidata,'%s\t%s\t%s\t%s\t%s\n','\DJ$_{\rm{ideal}}$','$N_{pa}$','Arch',...
        'Case \#','avg_fraction_numavgMW');
    
    
    for ncnt = 1:length(nfreearr) % begin nfree loop
        nval = nfreearr(ncnt);
        s2 = create_output_dirs(sprintf('./../../seq_analysis/overall/n_%d',nval));
        ...
        for arch_cnt = 1:length(arch_arr)  % begin arch loop
            ...
            fout_case = fopen(sprintf('./../../seq_analysis/n_%d/numavgMW_rcut_%s_pdifree_%g_%s.dat',...
            ...
            for casecntr = 1:length(casearr) % begin case loop
                ...
                ads_prefix = sprintf('adsfrac_numavg_rcut_%s_config_*.lammpstrj',cutoff);
                ...
                for fylcnt = 1:nfyles % begin running through all files of the given type
                    ...
                    if min(data(:,1)) > set_tmin
                        ...
                        nads_fracmon = sum(data(minindana:lendata,4));
                        sum_across_files = sum_across_files + nads_fracmon;
                        tot_cntr_across_files = tot_cntr_across_files + length(data(minindana:lendata,4));
