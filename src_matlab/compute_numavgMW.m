...
for pdi_cntr = 1:length(pdi_freearr) % begin pdi free loop
    ...
    fout_avg = fopen(sprintf('./../../outfiles/overall/numavgMW_ave_allcases_rcut_%s_pdifree_%g.dat',...
        cutoff,pdifree),'w');
    ...
    fout_sidata = fopen(sprintf('./../../outfiles/overall/sidata_numavgMW_rcut_%s_pdifree_%g.dat',...
        cutoff,pdifree),'w');
    ...
    for ncnt = 1:length(nfreearr) % begin nfree loop
        ...
        for arch_cnt = 1:length(arch_arr)  % begin arch loop
            ...
            s3 = create_output_dirs(sprintf('./../../outfiles/n_%d',nval));
            % Create case-based avg outfiles
            fout_case = fopen(sprintf('./../../outfiles/n_%d/numavgMW_rcut_%s_pdifree_%g_%s.dat',...
