...
for pdi_cntr = 1:length(pdi_freearr) % begin pdi free loop
    ...
    fnr   = fopen(sprintf('./../../seq_analysis/QnetBound_%s.txt',pdifree_str),'w'); % case wise net charge
    ...
    fmon  = fopen(sprintf('./../../seq_analysis/nmonlist_%s.txt',pdifree_str),'w'); % monomer distribution details
    fprintf(fmon,'%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n',...
        'N_pa','Arch','Case','N_gr','N_bb','N_ntbr','N_chbr','N_ntfr','N_chfr','Nposions','Nnegions','Not');
    fave  = fopen(sprintf('./../../seq_analysis/AvgQnetBound_%s.txt',pdifree_str),'w'); % average net charge
    ...
    for ncnt = 1:length(nfreearr) % begin nfree loop
        ...
        for arch_cnt = 1:length(arch_arr)  % begin arch loop
            ...
            for casecntr = 1:length(casearr) % begin case loop
                ...
                % each type
                inp_fylename = sprintf('./../../seq_analysis/data_all_dir/n_%d/%s/pdifree%s_pdigraft_%s/Case_%d/PEinitdata.txt',...
