...
% wherever necessary.
% Crude idea: if a polyanion monomer is within some cutoff of polycation
% graft, then it is counted as adsorbed. This definition is NOT used in the
% paper.
clear;
clc;
close all;
    ...
    for ncnt = 1:length(nfreearr) % begin nfree loop
        ...
        for arch_cnt = 1:length(arch_arr)  % begin arch loop
            ...
            for casecntr = 1:length(casearr) % begin case loop
                ...
                ads_prefix = sprintf('adsfracmon_rcut_%s_config_*.lammpstrj',cutoff);
